...
class DimenLogarithmic private constructor(
     ...
     */
    fun logsdpLoghdpLogwdpPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
    ...
    fun logsdp(context: AppDimensContext): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun loghdp(context: AppDimensContext): Float = px(context, DpQualifier.HEIGHT)
    fun logwdp(context: AppDimensContext): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun logsdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun loghdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun logwdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.WIDTH)
}