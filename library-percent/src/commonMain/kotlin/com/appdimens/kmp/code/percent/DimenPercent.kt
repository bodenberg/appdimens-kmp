...
class DimenPercent private constructor(
     ...
     */
    fun psdpPhdpPwdpPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
    ...
    fun psdp(context: AppDimensContext): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun phdp(context: AppDimensContext): Float = px(context, DpQualifier.HEIGHT)
    fun pwdp(context: AppDimensContext): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun psdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun phdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun pwdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.WIDTH)
}