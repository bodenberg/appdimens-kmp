/**
 * iOS Compose actuals: [AppDimensProvider], [currentScreenConfiguration],
 ...
 * window state (falling back to `UIScreen`).
 *
 ...
 *    The provider keys it with `remember(configuration)`: on a rotation/resize the
 ...
 *    O provider usa `remember(configuration)`: ao rotacionar/redimensionar a
...
internal fun contextFromConfiguration(configuration: ScreenConfiguration): AppDimensContext {
    ...
    return IosAppDimensContext(configuration, density, configuration.densityDpi.toFloat())
}
 ...
 *    from the live [LocalWindowInfo] window (not a frozen UIScreen snapshot), so
 *    the code-API fast lane stays resize-aware even without the provider. Falls
 *    back to the remembered UIScreen handle when no window info is available.
