/**
 * JVM platform actuals: thread-local metrics holder + weak identity map.
 */
...
/**
 * Weak-key identity map: values never retain their keys, so a window can be
 * collected normally (mirrors the Android `WeakHashMap` semantics).
 */
