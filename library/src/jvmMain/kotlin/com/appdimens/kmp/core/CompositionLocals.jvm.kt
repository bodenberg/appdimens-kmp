/**
 * Desktop Compose actuals: [AppDimensProvider], [currentScreenConfiguration],
 ...
 * [DimenMetrics] e o handle de janela [AppDimensContext] a partir da janela.
 */
/**
 * EN Builds the [AppDimensContext] from a live window [ScreenConfiguration].
 *    The provider keys it with `remember(configuration)`: on a window resize the
 *    configuration changes → a NEW context identity is produced → the
 *    [DimenCache] fast-window-slot identity check misses → metrics are rebuilt
 *    with fresh values. This mirrors Android, where the wrapped Context reads
 *    live `resources.configuration` on every resolution (self-healing fast lane).
 * PT Constrói o [AppDimensContext] a partir de um [ScreenConfiguration] vivo.
 *    O provider usa `remember(configuration)`: ao redimensionar a janela a
 *    configuração muda → nova identidade de contexto → a checagem de identidade
 *    do fast-window-slot do [DimenCache] erra → métricas são reconstruídas com
 *    valores novos. Espelha o Android, onde o Context embrulhado lê
 *    `resources.configuration` ao vivo em cada resolução (fast lane auto-curável).
 */
internal fun contextFromConfiguration(configuration: ScreenConfiguration): AppDimensContext {
    ...
    return DesktopAppDimensContext(configuration, density, configuration.densityDpi.toFloat())
}
...
/** EN Current [ScreenConfiguration] for this composition, from the desktop window. */
@Composable
 ...
 *    from the live [LocalWindowInfo] window (not a frozen AWT screen snapshot), so
 *    the code-API fast lane stays resize-aware even without the provider. Falls
 *    back to the remembered screen handle only when no window info is available.
 * PT Handle padrão para composições fora do [AppDimensProvider]: derivado da
 *    janela viva ([LocalWindowInfo]), não de um snapshot AWT congelado, para o
 *    fast lane da code API continuar ciente de resize mesmo sem provider.
 */
@Composable
