/**
 * macOS Compose actuals: [AppDimensProvider], [currentScreenConfiguration],
 ...
 * window state (falling back to `NSScreen`).
 *
 ...
 *    The provider keys it with `remember(configuration)`: on a resize the
 ...
 *    O provider usa `remember(configuration)`: ao redimensionar a configuração
 *    muda → nova identidade de contexto → o fast-window-slot do [DimenCache]
 *    erra → métricas são reconstruídas. Espelha o fast lane auto-curável do
 *    Android.
 */
internal fun contextFromConfiguration(configuration: ScreenConfiguration): AppDimensContext {
    ...
    return MacosAppDimensContext(configuration, density, configuration.densityDpi.toFloat())
}
 ...
 *    from the live [LocalWindowInfo] window (not a frozen NSScreen snapshot), so
 *    the code-API fast lane stays resize-aware even without the provider. Falls
 *    back to the remembered NSScreen handle when no window info is available.
 * PT Handle padrão para composições fora do [AppDimensProvider]: derivado da
 *    janela viva ([LocalWindowInfo]), para o fast lane da code API continuar
