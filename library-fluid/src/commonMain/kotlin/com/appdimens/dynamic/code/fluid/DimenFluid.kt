...
class DimenFluid private constructor(
     ...
     */
    fun fsdpFhdpFwdpPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
    ...
    fun fsdp(context: AppDimensContext): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun fhdp(context: AppDimensContext): Float = px(context, DpQualifier.HEIGHT)
    fun fwdp(context: AppDimensContext): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun fsdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun fhdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun fwdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.WIDTH)
}