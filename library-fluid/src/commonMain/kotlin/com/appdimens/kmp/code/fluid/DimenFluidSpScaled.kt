...
class FluidSp private constructor(
     ...
     */
    fun fsspFhspFwspPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
     ...
     */
    fun fsemFhemFwemPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
    ...
    fun fssp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.SMALL_WIDTH)
    fun fhsp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.HEIGHT)
    fun fwsp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.WIDTH)

    /** EN Resolve final value in pixels (WITHOUT font scale). */
    fun fsem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.SMALL_WIDTH, fontScaleOverride = false)
    fun fhem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.HEIGHT, fontScaleOverride = false)
    fun fwem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.WIDTH, fontScaleOverride = false)
}