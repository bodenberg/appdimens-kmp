...
class DensitySp private constructor(
     ...
     */
    fun dsspDhspDwspPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
     ...
     */
    fun dsemDhemDwemPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
    ...
    fun dssp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.SMALL_WIDTH)
    fun dhsp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.HEIGHT)
    fun dwsp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.WIDTH)

    /** EN Resolve final value in pixels (WITHOUT font scale). */
    fun dsem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.SMALL_WIDTH, fontScaleOverride = false)
    fun dhem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.HEIGHT, fontScaleOverride = false)
    fun dwem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.WIDTH, fontScaleOverride = false)
}