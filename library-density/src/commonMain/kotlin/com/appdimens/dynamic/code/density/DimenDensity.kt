...
class DimenDensity private constructor(
     ...
     */
    fun dsdpDhdpDwdpPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
    ...
    fun dsdp(context: AppDimensContext): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun dhdp(context: AppDimensContext): Float = px(context, DpQualifier.HEIGHT)
    fun dwdp(context: AppDimensContext): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun dsdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun dhdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun dwdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.WIDTH)
}