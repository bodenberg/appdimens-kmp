...
class DimenFit private constructor(
     ...
     */
    fun ftsdpFthdpFtwdpPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
    ...
    fun ftsdp(context: AppDimensContext): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun fthdp(context: AppDimensContext): Float = px(context, DpQualifier.HEIGHT)
    fun ftwdp(context: AppDimensContext): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun ftsdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun fthdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun ftwdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.WIDTH)
}