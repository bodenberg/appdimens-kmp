...
class DimenInterpolated private constructor(
     ...
     */
    fun isdpIhdpIwdpPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
    ...
    fun isdp(context: AppDimensContext): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun ihdp(context: AppDimensContext): Float = px(context, DpQualifier.HEIGHT)
    fun iwdp(context: AppDimensContext): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun isdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun ihdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun iwdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.WIDTH)
}