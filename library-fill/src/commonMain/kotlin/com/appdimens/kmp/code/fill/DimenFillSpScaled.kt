...
class FillSp private constructor(
     ...
     */
    fun flsspFlhspFlwspPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
     ...
     */
    fun flsemFlhemFlwemPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
    ...
    fun flssp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.SMALL_WIDTH)
    fun flhsp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.HEIGHT)
    fun flwsp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.WIDTH)

    /** EN Resolve final value in pixels (WITHOUT font scale). */
    fun flsem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.SMALL_WIDTH, fontScaleOverride = false)
    fun flhem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.HEIGHT, fontScaleOverride = false)
    fun flwem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.WIDTH, fontScaleOverride = false)
}