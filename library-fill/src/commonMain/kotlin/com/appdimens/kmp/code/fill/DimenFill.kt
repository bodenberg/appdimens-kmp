...
class DimenFill private constructor(
     ...
     */
    fun flsdpFlhdpFlwdpPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
    ...
    fun flsdp(context: AppDimensContext): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun flhdp(context: AppDimensContext): Float = px(context, DpQualifier.HEIGHT)
    fun flwdp(context: AppDimensContext): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun flsdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun flhdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun flwdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.WIDTH)
}