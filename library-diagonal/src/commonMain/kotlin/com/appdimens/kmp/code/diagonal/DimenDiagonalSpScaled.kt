...
class DiagonalSp private constructor(
     ...
     */
    fun dgsspDghspDgwspPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
     ...
     */
    fun dgsemDghemDgwemPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
    ...
    fun dgssp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.SMALL_WIDTH)
    fun dghsp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.HEIGHT)
    fun dgwsp(context: AppDimensContext): Float = resolvePx(context, DpQualifier.WIDTH)

    /** EN Resolve final value in pixels (WITHOUT font scale). */
    fun dgsem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.SMALL_WIDTH, fontScaleOverride = false)
    fun dghem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.HEIGHT, fontScaleOverride = false)
    fun dgwem(context: AppDimensContext): Float = resolvePx(context, DpQualifier.WIDTH, fontScaleOverride = false)
}