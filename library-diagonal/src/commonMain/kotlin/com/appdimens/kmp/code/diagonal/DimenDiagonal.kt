...
class DimenDiagonal private constructor(
     ...
     */
    fun dgsdpDghdpDgwdpPx(context: AppDimensContext): Triple<Float, Float, Float> {
        val configuration = context.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
    ...
    fun dgsdp(context: AppDimensContext): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun dghdp(context: AppDimensContext): Float = px(context, DpQualifier.HEIGHT)
    fun dgwdp(context: AppDimensContext): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun dgsdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun dghdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun dgwdpBase(context: AppDimensContext): Float = resolveDp(context, DpQualifier.WIDTH)
}